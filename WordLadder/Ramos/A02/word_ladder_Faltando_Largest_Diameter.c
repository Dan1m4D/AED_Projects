...
static hash_table_t *hash_table_create(void)
{
  ...
  hash_table->heads = (hash_table_node_t**) malloc(hash_table->hash_table_size*sizeof(hash_table->heads)); 
...
static void hash_table_grow(hash_table_t *hash_table)
{
  ...
  hash_table_node_t **new_heads = (hash_table_node_t**) malloc(hash_table->hash_table_size*sizeof(hash_table->heads));  
  memset(new_heads, 0, hash_table->hash_table_size*sizeof(hash_table->heads));
...
static int connected_component_diameter(hash_table_t *hash_table, hash_table_node_t *source)
{
  hash_table_node_t *node, 
                    *goalRepresentative,
  int diameter = 0;
  ...
  for (unsigned int x = 0; x < hash_table->hash_table_size; x++) {
    ...
    for(node = hash_table->heads[x];node != NULL;node = node->next) {
      ...
      //Identificar o representative do node de desitino
      goalRepresentative = find_representative(node);

      //Ir para a próxmima iteração se ambos n fizerem parte do mesmo componente conexo
      ...
      if (sourceRepresentative == find_representative(node)) {
        ...
        distance = breadth_first_search(hash_table->hash_table_size, currentPath, source, node);
...
static int largest_connected_component_diameter(hash_table_t *hash_table){
  hash_table_node_t *node;
  int current_largest_diameter = 0, current_diameter = 0;
  unsigned int index;

  
  for (index  = 0u ; index < hash_table->hash_table_size ; index++)
  {
    if(hash_table->heads[index] == NULL){
      continue;
    }      

    node = hash_table->heads[index] ;

    current_diameter = connected_component_diameter(hash_table, node);

    if( current_diameter > current_largest_diameter ){
      current_largest_diameter = current_diameter ;
    }
    
  }

  return current_largest_diameter ;
}

/*
static int largest_connected_component_diameter(hash_table_t *hash_table){
  hash_table_node_t *node;
  const hash_table_node_t *searchRepresentative;

  int largest_diameter = 0, current_diameter = 0, distance;
  unsigned int index;

  //node = hash_table->heads[index];
  
  for (index  = 0 ; index < hash_table->hash_table_size ; index++)
  {
    if(hash_table->heads[index] == NULL){
      continue;
    }      
    
    searchRepresentative = find_representative(hash_table->heads[index]);

    for( node = hash_table->heads[index] ; node != NULL ; node->next )
    {
      if(searchRepresentative ==  find_representative(node)){
...
static void path_finder(hash_table_t *hash_table,const char *from_word,const char *to_word)
{
  ...
  if (sourceRepresentative != goalRepresentative) {
    printf("\nERRO! As palavras inseridas não tem ligações possíveis!!! (Componentes Conexos diferentes)\n");
    return;
  }
  
  hash_table_node_t *currentPath[sourceRepresentative->number_of_vertices + 1];

  (void)breadth_first_search(sourceRepresentative->number_of_vertices, currentPath, source, goal);
  
  int nivel = 0;
  for (hash_table_node_t* parentNode = goal; parentNode != NULL; parentNode = parentNode->previous) {
    printf("\n--> %s", parentNode->word);
    nivel++;
  }
...
int main(int argc,char **argv)
{
  ...
  //Calculating the largest diameter of all connected components :
  largest_diameter = largest_connected_component_diameter(hash_table);
